...
use strict          ;
use warnings        ;
require "lib/Aliscore_module.pm" ;
use Tie::File       ;
use Fcntl           ;
...
open OUT, ">", "${file}_Profile\-l${level}.txt" and last OUTFILE if $level   =~  /\d+/;
open OUT, ">", "${file}_Profile\-l_all.txt"     and last OUTFILE if $level   !~  /\d+/ && $tree=~/\w/;
...
LIST: 		 {
	...
	open LIST, ">", "${file}_List\-l${level}.txt" 		      and last LIST 		if $level  =~ /\d+/;
	open LIST, ">", "${file}_List\-l_all.txt"     		      and last LIST 		if $level  !~ /\d+/&& $tree=~/\w/;
